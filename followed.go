...
func getFollowedFeedView(renderer *lipgloss.Renderer, db *sql.DB, user SavedUser) (FollowedFeedModel) {
	infoWidth := 20
	...
	posts, err := FindFollowedPosts(db, user)
	if err != nil {
		log.Error(err)
	}
...
func (m FollowedFeedModel) Init() tea.Cmd {
	return nil
}

func (m FollowedFeedModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
...
func (m FollowedFeedModel) View() string {
